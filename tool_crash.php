...
if($SQLObj0->LinkNext())
{
	...
	$str = "DELETE FROM player_record_log WHERE `datetime`>='".$today." 00:00:00' AND `datetime`<='".$today." 23:59:59'";
	echo $str."<BR>";
	$SQLObj0->RunSQL($str);
}

//檢查是否已經更新過了
$str = "SELECT * FROM myteam_score_log WHERE `datetime`>='".$today." 00:00:00' AND `datetime`<='".$today." 23:59:59'";
$SQLObj0->RunSQL($str);
if($SQLObj0->LinkNext())
{
	echo $str."<BR>";
	echo "今日已經更新過價錢了(3)!!!<BR>";
	$str = "DELETE FROM myteam_score_log WHERE `datetime`>='".$today." 00:00:00' AND `datetime`<='".$today." 23:59:59'";
	echo $str."<BR>";
	$SQLObj0->RunSQL($str);
}
...
for($i=0;$i<$allteamcount;$i++)
{
	$teamid = $i+1;
	//取得名單
	$str = "SELECT * FROM myteam_roster_log WHERE team_id=".($i+1)." AND `datetime`<='".$date." 19:00:00' ORDER BY `datetime` DESC LIMIT 0,1";
	//if($i==6)
		//echo "<BR>".$str."<BR>";
	$SQLObj0->RunSQL($str);
	if($SQLObj0->LinkNext())
	{
		...
		$mktime = gmmktime(19,0,0,$mm,$dd,$yy);
		$today = $date;
		$str = "SELECT * FROM myteam_score_log WHERE teamid= = '".($i+1)."' AND `datetime`>='".$today." 00:00:00' AND `datetime`<='".$today." 23:59:59' AND teamid=".($i+1);
		$SQLObj0->RunSQL($str);
		//今日未變動
		if(!$SQLObj0->LinkNext())
		{
...
//是否已經更新過了
$str = "INSERT INTO log_pricemove SET `datetime` = '".$datetime."',teamcount=1000";
$SQLObj0->RunSQL($str);
?>
