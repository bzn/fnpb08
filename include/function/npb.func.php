...
define("AUTOMAXMEMBER",20);
define("MAXMEMBER",100);

//取得聯盟資料
function GetLeaInfoByMyteamID($SQLObj0,$myteamid)
{
	$str = "SELECT * FROM lea_index WHERE team_id ='".$myteamid."'";
	$SQLObj0->RunSQL($str);
	if($SQLObj0->LinkNext())
	{
		$leaid = $SQLObj0->GetData("LEA_ID");	
		$str = "SELECT * FROM lea_data WHERE lea_id = '".$leaid."'";
		$SQLObj0->RunSQL($str);
		$SQLObj0->LinkNext();
		$a_leainfo['id'] = $SQLObj0->GetData("ID");
		$a_leainfo['name'] = $SQLObj0->GetData("NAME");
		return $a_leainfo;
	}		
}

//取得lea人數
function GetLeaCountByMyteamID($SQLObj0,$myteamid)
{
	$str = "SELECT * FROM lea_index WHERE team_id ='".$myteamid."'";
	$SQLObj0->RunSQL($str);
	if($SQLObj0->LinkNext())
...
{
	$str = "SELECT points FROM player_record_".THISYEAR." WHERE player_id = ".$id;
	$SQLObj0->RunSQL($str);
	if($SQLObj0->LinkNext())
		return $SQLObj0->GetData('points');
	else 
...
function GetPlayerDataByPeri($SQLObj0,$id,$peri)
{
	...
	switch ($peri)
	{
		...
		case 7:
		{
			$str = "SELECT * FROM player_record_7 WHERE player_id='".$id."'";
			break;	
		}
		case 15:
		{
			$str = "SELECT * FROM player_record_15 WHERE player_id='".$id."'";
			break;
		}
		case 30:
		{
			$str = "SELECT * FROM player_record_30 WHERE player_id='".$id."'";
			break;
		}
		case 365:
		{
			$str = "SELECT * FROM player_record_".THISYEAR." WHERE player_id='".$id."'";
			break;
		}
		default:
		{
			$str = "SELECT * FROM player_record_".$peri." WHERE player_id='".$id."'";
			break;
		}
	}
	//echo $str;
