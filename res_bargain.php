...
//Pither//
$sqlstr = '
SELECT (C.`points` / (A.`price` / 100) ) AS `bargain`, C.`points`, A.`price`,B.`name_2`,B.`id` 
...
WHERE 1 AND B.`p` = 1 
ORDER BY `bargain` DESC LIMIT 0,10';
...
//Pither//
$sqlstr = '
SELECT (A.`points` / (A.`price` / 100) ) AS `bargain`, A.`points`, A.`price`,B.`name_2`,B.`id` 
FROM `'.$ini_array['system']['dbname'].'`.`player_record_2008` AS A LEFT JOIN 
...
<div id="headerSection">
    <span class="pageTitle">物超所值</span>
    <p class="dottedLine">&nbsp;</p>
    <div class="v12">以下是物超所值的球員 *PPM(Points Per Million) = 積分 / 價錢</div>
</div>  
<p class="line">&nbsp;</p><br>
