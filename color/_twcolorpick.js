/************************************************
 * Copyright (c)2007 RandyLien (randy@ohdada.com)
 * TWColorPick.js version 0.80 2007-05-15
 * This script is licensed under the MIT and GPL licenses.
 * That means you can use this code free but don't claim to have written it yourself!
 * Thanks for Matt Kruse's cool position.js helping my colorpick can get the correct position
 * on different browser!
 *
 * How to Use ?
 * 1.include this file , twcolorpick.css , position.js and Prototype.js
 * 2.put this string in your <head> </head>
 *	if($('gridBox'))
 *	{
 * 		Event.observe('gridBox','click',colorpick,false);
 *	}
 * 3.call function showColorPick(event , YOUR BUTTON ID,YOUR INPUT ID);
 * 4.If you get any error please tell me ! Thanks!
 * 
 * http://developer-randy.blogspot.com
 * 
 * 
************************************************/
	...
	function colorpick(e)
	{
		...
		if(child.className=="colorBox"){
			...
			colorStyle = rgbToHex(colorStyle);
			bgcolor.value = colorStyle;
		}
		Event.stop(e);
	}
	
	function showColorPick(e,source,target)
	{
		if(ie) e = event;
		sourceObj = $(source);
		...
		if(showpick)
		{
			...
			var x =Position.get(source_button).left;
			var y =Position.get(source_button).top;
