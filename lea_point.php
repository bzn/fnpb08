...
{
	$a_id = $SQLObj0->GetData('TEAM_ID');
}

for($i=0;$i<$nrows;$i++)
{
	$str = "SELECT * FROM myteam_data WHERE id='".$a_id[$i]."'";
	$SQLObj0->RunSQL($str);
	if($SQLObj0->LinkNext())
	{	
		$a_teamname[$i] = $SQLObj0->GetData('NAME');
		$a_point[$i] = $SQLObj0->GetData('POINTS');
		$a_money[$i] = $SQLObj0->GetData('MONEY');
	}
}

array_multisort($a_point,SORT_DESC,SORT_NUMERIC,$a_id,$a_teamname,$a_money);
...
<div id="headerSection"><span class="pageTitle">積分排行榜</span><br/>
</div>
<p class="line">&nbsp;</p><br>
...
<table class="v12 infotable">
     ...
     <?php
		...
		?>
            <td class="tleft border nowrap"><?php echo $nowrank;?></td>
